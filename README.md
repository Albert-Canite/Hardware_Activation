...
- `train_and_validate.py`：启动器（只做预检查与崩溃归因）
- `train_worker.py`：真正训练与验证逻辑
...
脚本最开始还会执行 `[BOOT] preflight import probe`，先在子进程里验证 `numpy/torch/openpyxl` 能否被安全导入。
若这里就报 `0xC0000005`，说明崩溃发生在**训练前的原生库导入阶段**，不是模型训练逻辑。
